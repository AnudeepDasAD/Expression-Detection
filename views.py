...
class VideoCamera(object):
	def __init__(self):
		...
		cascade_classifier = os.path.join(current_path, 'haarcascade_frontalface_alt2.xml')
		self.face_cascade = cv2.CascadeClassifier(cascade_classifier)
		self.recognizer = cv2.face.LBPHFaceRecognizer_create()
		
		emotion_training = os.path.join(current_path, 'emotion_training.yml')
		emotion_labels = os.path.join(current_path, 'emotion_labels.pkl')
		self.recognizer.read(emotion_training)
		...
		with open(emotion_labels, "rb") as f:
...
def index(request):
	return render(request, 'Detector/indexDjango.html')
